...
class SphincsC:
    def __init__(self):
        ...
        self._MAX_HASH_TRIALS_FORS = (
            1 << (self._SPX_FORS_ZERO_LAST_BITS + 10))
    ...
    def verify(self, m, sig, pk):
        ...
        for i in range(self._n + (self._k - 1) * (self._a + 1) * self._n,
                       self._n + (self._k - 1) * (self._a + 1) * self._n +
                       (self._h + self._d * self._len_1) * self._n,
                       self._n):
    ...
    def xmss_pk_gen(self, secret_seed, public_key, adrs: ADRS):
        pk = self.treehash(secret_seed, 0, self._h_prime,
                           public_key, adrs.copy())
        return pk
    ...
    def xmss_sign(self, m, secret_seed, idx, public_seed, adrs):
        ...
        for j in range(0, self._h_prime):
            ...
            auth += [self.treehash(secret_seed, ki * 2 **
                                   j, j, public_seed, adrs.copy())]
    ...
    def xmss_pk_from_sig(self, idx, sig_xmss, m, public_seed, adrs, counter):
        ...
        node_0 = self.wots_pk_from_sig(
            sig, m, public_seed, adrs.copy(), counter)
        node_1 = 0
        ...
        for i in range(0, self._h_prime):
            ...
            if math.floor(idx / 2**i) % 2 == 0:
                ...
                node_1 = self.hash(public_seed, adrs.copy(),
                                   node_0 + auth[i], self._n)
            else:
                adrs.set_tree_index((adrs.get_tree_index() - 1) // 2)
                node_1 = self.hash(public_seed, adrs.copy(),
                                   auth[i] + node_0, self._n)
    ...
    def wots_pk_gen(self, secret_seed, public_seed, adrs: ADRS):
        ...
        for i in range(0, self._len_x):
            ...
            tmp += bytes(self.chain(sk, 0, self._w -
                         1, public_seed, adrs.copy()))
    ...
    def wots_pk_from_sig(self, sig, m, public_seed, adrs: ADRS, counter=0):
        ...
        for i in range(0, self._len_x):
            adrs.set_chain_address(i)
            tmp += self.chain(sig[i], msg[i], self._w -
                              1 - msg[i], public_seed, adrs.copy())
        ...
        if counter[0] != 0:
            ...
            while found_flag:
                ...
                if counter[0] > self._MAX_HASH_TRIALS_FORS:
                    return -1
                counter_bytes = int.to_bytes(
                    counter[0], self._COUNTER_SIZE, 'big')
                buf = self.hash_with_counter(
                    r, public_seed, public_root, value, counter_bytes, digest_size=self._n)
                zero_bits = int.from_bytes(buf, 'big') & mask
                if zero_bits == 0:
    ...
    def base_w(self, x, w, out_len):
        ...
        for consumed in range(0, out_len):
            ...
            basew.append((total >> max(bits, 0)) % w)
            vout += 1
    ...
    def fors_pk_gen(self, secret_seed, public_seed, adrs: ADRS):
        ...
        root = bytes()
        for i in range(0, self._k - 1):
            root += self.fors_treehash(secret_seed,
                                       i * self._t, self._a, public_seed, adrs)
        ...
        for i in range(0, self._k - 1):
            ...
            for j in range(0, self._a):
                ...
                if math.floor(idx / 2**j) % 2 == 0:
                    adrs.set_tree_index(adrs.get_tree_index() // 2)
                    node_1 = self.hash(
                        public_seed, adrs.copy(), node_0 + auth[j], self._n)
                else:
                    adrs.set_tree_index((adrs.get_tree_index() - 1) // 2)
                    node_1 = self.hash(
                        public_seed, adrs.copy(), auth[j] + node_0, self._n)
    ...
    def spx_sign(self, m, secret_key):
        ...
        md_int = int.from_bytes(tmp_md, 'big') >> (
            len(tmp_md) * 8 - self._k * self._a)
